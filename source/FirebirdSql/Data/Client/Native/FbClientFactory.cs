...
namespace FirebirdSql.Data.Client.Native
{
	...
	internal static class FbClientFactory
	{
...
#if	(!LINUX)
        private static string LocateValidDllPath(string privateFolder, string dllName)
        {
            var dllPath = Path.Combine(privateFolder, dllName);
            if (File.Exists(dllPath))
                return dllPath;

            dllPath = Path.Combine(Path.Combine(privateFolder, ProcessorArchitecture), dllName);
            if (File.Exists(dllPath))
                return dllPath;
        ...
        private static string GetDefaultDllPath()
        {
            ...
            {
                var dllName = Path.ChangeExtension(DefaultDllName, "dll");
                var dllPath = LocateValidDllPath(Path.GetDirectoryName(localPath), dllName);
                if (string.IsNullOrEmpty(dllPath))
                {
                    Assembly entryAssembly = Assembly.GetEntryAssembly();
                    if (entryAssembly != null)
                    {
                        dllPath = LocateValidDllPath(Path.GetDirectoryName(entryAssembly.Location), dllName);
                    }
                }

                return string.IsNullOrEmpty(dllPath) ? DefaultDllName : dllPath;
            }
            else
            {
                return DefaultDllName;
